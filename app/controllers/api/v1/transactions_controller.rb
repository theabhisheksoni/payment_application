class Api::V1::TransactionsController < ApplicationController
  ...
  before_action :set_parent_transaction, :set_status, only: %i[charge_transaction refund_transaction reversal_transaction]
  ...
  def refund_transaction
    if @parent_transaction.is_charged_and_approved?
      refund_transaction = @parent_transaction.refund_transactions.new(update_transaction_params.merge(status: @status))
  ...
  def transaction_params
    params.require(:transaction).permit(:amount, :customer_email, :customer_phone).merge(merchant_id: current_user.id, status: 'approved')
  end
